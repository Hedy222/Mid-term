...
strawb <- read_xlsx("~/Desktop/Strawberries/strawberries-2022oct30-a (1).xlsx",col_names = TRUE)
...
Data_Q2 <- filter(strawb %>% filter(Year == "2016", State == "CALIFORNIA"), Domain=="ORGANIC STATUS")
View(Data_Q2)

range <- 231304956*1.96*0.137
lower <- 231304956-range
upper <- 231304956+range


#3

Data_Q3 <- filter(strawb %>% filter(Year == "2016", State == "CALIFORNIA"), Domain=="Total")
View(Data_Q3)
...
#length(chemical_types)
chemical_names <- unique(strawb_chem$chem_name)
length(chemical_names)

#unique(fertilizers$chem_name)
#unique(Strawberries$chem_name)
length(chemical_names)+3


#5

CA <- filter(strawb_chem, State == "CALIFORNIA")
FLO <- filter(strawb_chem, State == "FLORIDA")
